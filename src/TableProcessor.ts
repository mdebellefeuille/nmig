...
export async function createTable(conversion: Conversion, tableName: string): Promise<void> {
    ...
    const tableColumnsSql: string = columns
        .data
        .map((column: any) => {
        ...
        .concat([`"${ conversion._schema }_${ originalTableName }_data_chunk_id_temp" BIGINT`])
        .join(',');

    const sqlCreateTable: string = `CREATE TABLE IF NOT EXISTS "${ conversion._schema }"."${ tableName }"(${ tableColumnsSql });`;
